...
def sign_refresh(sub: str) -> tuple[str, str]:
    ...
    return jwt.encode(payload, env.JWT_REFRESH_SECRET, algorithm="HS256"), jti


def verify_access(token: str) -> AccessPayload:
    try:
        data = jwt.decode(token, env.JWT_ACCESS_SECRET, algorithms=["HS256"])
...
def verify_refresh(token: str) -> RefreshPayload:
    try:
        data = jwt.decode(token, env.JWT_REFRESH_SECRET, algorithms=["HS256"])
